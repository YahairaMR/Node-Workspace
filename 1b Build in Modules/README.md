...
17. Determine if a file is from a directory

`fs.statSync()` provides us with a set of information for a given file. We can utilize the `.isDirectory()` method to check if a file (or file path) is actually a directory.
